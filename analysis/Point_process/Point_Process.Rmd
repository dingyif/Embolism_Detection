...
for(folder_i in 1:length(all_folders_name_short)){
  ...
  if(hawkes.par$convergence!=0){
    ...
    not_conv_list <- c(not_conv_list,folder_name_i)
    HP_param_i <- c(folder_i, species_idx,n_emb_img,rep(0,5))
    ...
    HP_param_i <- c(folder_i, species_idx,n_emb_img,HP_lam0,HP_alpha,HP_beta,HP_gamma,1)
    all_STHP_param <- rbind(all_STHP_param,HP_param_i)
...
colnames(all_STHP_param) <- c("folder_index","species","num_emb_img","lambda0","alpha","beta","gamma","converge")
...
for(folder_i in 1:length(all_folders_name_short)){
  ...
  if(length(i_ST)>0 && length(i_HP)>0){#found in both all_STHP_pear_res,all_HP_pear_res
    ...
    fig_boxplot <- ggboxplot(both_pear_res, x = "model", y = "pearson_res", 
          color = "model", palette = c("#00BFC4","#F8766D"),
          order = c("Temporal", "Spatiotemporal"),
          ylab = "Pearson Residuals", xlab = "Models")+ggtitle("Pearson Residuals Box Plot")
...
+ Two-sided: whether the mean difference (of STHP-HP paired value) is equal to 0?

+ One-sided: whether the mean difference is less than 0? (alternative hypothesis is mean difference greater than 0)
...
ztable(t_test_pvals,caption="Change in P-values of Paired T-test (Temporal vs Spatial-Temporal Hawkes Process)") %>%
    addRowColor(rows=1,bg="#C90000",color="white") %>%
